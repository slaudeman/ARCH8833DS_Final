...
//console.log('current color:' + clrRGBA);

//colorchange will be of above. Change getElementById to selected element
//same methodologies below, ea function just needs to be altered for selected object, within the svg

///////////////////////////////////////////////////////////////////////

//need to pull drag/drop in!

var targetElement = evt.target;
var targetObj

function getTargetElementId() {
	if (backdrop != targetElement && interface != targetElement) {
		targetObj = targetElement;
	}
}

function changeLineweightLight() {
	targetObj.setAttributeNS(null, 'stroke-width', 1);
}

function changeLineweightMed() {
	targetObj.setAttributeNS(null, 'stroke-width', 2);
}

function changeLineweightHeavy() {
	targetObj.setAttributeNS(null, 'stroke-width', 3);
}

function changeStrokeClr() {
	targetObj.setAttributeNS(null, 'stroke', clrRGB);
}

function changeObjClr() {
	targetObj.setAttributeNS(null, 'fill', clrRGBA);
}

function scaleObj() {
	targetObj.setAttributeNS(null, 'transform', 'scale(' + scaleVal + ')');
}

function rotateObj() {
	targetObj.setAttributeNS(null, 'transform', 'rotate(' + rotateDeg + ')');
}